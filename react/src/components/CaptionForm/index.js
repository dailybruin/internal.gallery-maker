...
function CaptionImagePair(props){
   ...
   // }
   // const caption = useSelector(selectCaption); // for retrieving state

   const [caption, setCaption] = useState(props.initialCaption)
   const [credit, setCredit] = useState(props.initialCaption)
...
function CaptionsForm() {
	...
	return (
		<div>
			...
			{gallery.map(img => <CaptionImagePair key={img.url} img_url={img.url} initialCaption={img.caption}/>)}
